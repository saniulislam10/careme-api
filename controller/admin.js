 ...
 exports.getAllCustomers = async (req, res, next) => {
    ...
    try {
        ...
        res.status(200).json({
            data: data,
            count: count,
            message: 'Successfully Get All Customers info.'
        })
    } catch (err) {
        console.log(err)
        if (!err.statusCode) {
...
exports.getSelectedUserDetails = async (req,res,next)=>{
    try {
      const selectedIds= req.body.selectedIds;
  
      const data = await User.find({ _id: { $in: selectedIds}})
